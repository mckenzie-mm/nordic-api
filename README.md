...
The app runs on an AWS container service (ECS). This was chosen instead of Kubernetes due to the lower cost and ease with which a demonstration setup could be configured. It is deployed with infra-structure as a code (CDK). It is currently deployed only on AWS region located to Australia and will be slow to load in Europe/Norway. 
